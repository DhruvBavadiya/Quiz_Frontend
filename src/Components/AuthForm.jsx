...
const AuthForm = ({ isLogin }) => {
  ...
  // Check if the user is new (signup)
  ...
  const validationSchema = Yup.object().shape({
    ...
    username: isNewUser
      ...
      : Yup.string(),
    mobile: isNewUser
      ? Yup.number().typeError("Mobile must be a number").positive("Mobile must be positive").integer("Mobile must be an integer")
      : Yup.number(),
  });
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#282828]">
      ...
      <form onSubmit={formik.handleSubmit} className="w-96 p-8 bg-white shadow-md rounded">
        <h1 className="text-3xl font-semibold mb-8 text-[#282828]">
          {isLogin ? "Welcome back to Quizzy!" : "Join Quizzy now!"}
        </h1>
